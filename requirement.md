...
- **fix: 专辑封面全线占位——两个独立缺口叠加，任何一个单独修都没用** — 生产环境实测确认全部 5 张专辑的 `GET /api/v1/catalog/albums/{id}/artwork` 无一例外返回 `{"error":{"code":"no_artwork","message":"no artwork"}}`。这个 gap 早在 v5.37.1 就诊断记录过（"服务端库里根本没有封面数据……是媒体摄取的缺口，按路线图属于 v6"），本次把它从 v6 提前排入 v5.39.0 修复。完整诊断分两半，缺一不可：
- **缺口一：从来没有任何东西写过封面** — `Album.ArtworkMediaObjectID`（`internal/catalog/types.go:45`）字段存在、持久化路径存在（`postgres/repository.go`）、更新入口也存在（`PATCH /admin/catalog/albums/{id}`），但 `ImportTrack`（`internal/catalog/service.go:435`）是纯元数据操作——调用方直接提供 title/artist/album 字符串，服务端从未打开过音频文件本身。没有任何扫描/发现端点，`go.mod` 里此前也从未有过音频元数据解析库。FLAC 文件本身元数据齐全（stream 端点前几百字节能直接读出 album/ALBUMARTIST/artist/composer/ISRC/UPC 等完整 Vorbis comment），说明音频文件里其实有内嵌封面，只是入库时从未被读取过。
- **缺口二：就算封面链接对了，当前部署形态也没有能力送出字节** — `getAlbumArtwork`（`internal/httpapi/handler_catalog.go`）产生 URL 此前只有一条路：`handler.storage.GeneratePresignedURL`。而 `InferCapabilities`（`internal/storage/validation.go:52`）对 `BackendTypeLocal` 只给 `ServerRangeReads: true`，`PresignedURLs` 恒为 `false`。生产环境正是 `backendId: "local-music"`、`backendType: "local"`——`GeneratePresignedURL` 必然返回 `ErrProbeUnsupported`，端点必然答 503 `presign_failed`，哪怕 `ArtworkMediaObjectID` 真的填对了也一样。音轨播放没有这个问题，是因为它通过 `internal/streamsign` 的 HMAC 签名 URL 走 API 代理字节；封面此前完全没有等价路径。
- **设计约束——为什么不是"入库时补提取"** — 用户已明确表态生产数据库不会再被删除重建，这条修复必须让**已经导入的**曲库直接出封面，不要求任何重新扫描/重新导入/数据库重建，这条约束直接排除了"在 ImportTrack 里顺手加提取逻辑"作为主要手段（那只对未来新导入的曲目生效）。媒体挂载可能是只读的（`LocalConfig.ReadOnly`，v5.35.0 为此加的字段），绝不能往媒体后端写缓存文件。因此落地方案是：**按需解析封面，直接读音频文件，走 API 代理**——完全复刻音轨流媒体已经在本地后端上验证过的模式。
- **解析优先级** — 当预签名路径不可用、或 `ArtworkMediaObjectID` 为空时（这两个条件本质是同一件事的两种触发方式：没法产出预签名 URL），从该专辑的曲目里挑一首（按 discNumber 再按 trackNumber 从低到高，最后以 track ID 兜底，保证结果与仓库返回顺序无关地完全确定）依次尝试：**先**读音频文件内嵌图片，**再**找同目录下的封面文件（`cover`/`folder`/`front`/`album`，大小写不敏感，`.jpg`/`.jpeg`/`.png`/`.webp`）——后者在翻录曲库里极常见，成本几乎为零。两者都找不到才诚实返回 404 `no_artwork`，服务端绝不合成占位图（客户端已经有正确的占位音符图标兜底，伪造图片只会更糟）。`ArtworkMediaObjectID` 已设置且后端支持预签名（S3）这个组合的行为原样保留，与今天完全一致。
- **库选型：`github.com/dhowden/tag`** — 覆盖 FLAC/OGG-Vorbis/MP4/ID3v2(MP3)/ID3v1/DSF 全部内嵌图片解析，`Metadata.Picture()` 直接给出 `MIMEType`+`Data`。**许可证更正**：任务原始描述称其为 MIT，实测该模块 `LICENSE` 文件是 **BSD-2-Clause**（无"不得用于背书"条款的二条款版本），并非 MIT——两者同属宽松、可商用、GPL 兼容的许可证族，不影响选型结论，但如实记录这处更正。选型前实测验证了它对本任务的适配度：FLAC reader（`flac.go`）只要求 `"fLaC"` 魔数后跟任意数量的元数据块、以一个标了 last-block 位的块收尾，**不要求 STREAMINFO 存在或在前**——这让测试夹具可以纯手工拼字节（`internal/artwork/artwork_test.go` 的 `buildFLACWithPicture`），不需要下载真实音频或依赖外部编码器。
- **新增 `internal/artwork` 包** — 纯函数、零 HTTP/存储依赖：`ExtractEmbedded(path)` 用 `dhowden/tag` 解内嵌图；`FindSibling(dir)` 单次 `os.ReadDir` 建立大小写不敏感查找表，按优先级匹配候选文件名。两者对"识别不出格式/没有图片"都返回 `(_, false, nil)`（不是错误，是符合预期的常见结果），只有真实 I/O 失败才返回 error。
- **`internal/storage` 新增 `Service.LocalPath`** — 复用 `GetObject`/`PutObject` 已有的 `localRootPath`+`SafeObjectPath` 组合，解析 (backendId, objectKey) 到绝对文件路径，本地/NFS/SMB 之外的后端（如 S3）返回 `ErrProbeUnsupported`。`resolveAlbumArtwork` 靠它拿到音频文件路径，再用 `filepath.Dir` 得到同目录去找 sibling 文件。
- **泛化 `internal/streamsign`，而不是另起一套签名** — `Signer.Sign`/`Verify` 的参数从"trackID"改名为"resourceID"（纯语义重命名，HMAC 计算逻辑一字未改，`Sign(track.ID)`/`Verify(trackID, exp, sig)` 两处既有调用点行为完全不变）。新的专辑封面签名 URL 用 `"album:"+albumId` 作为签名负载而非裸 ID，确保就算音轨 ID 和专辑 ID 字符串恰好相撞，一个资源的签名 URL 也永远不能拿去访问另一个资源——`streamsign_test.go` 新增的 `namespaced resource ID does not collide with bare ID` 用例专门锁住这条性质。
- **新端点 `GET /api/v1/catalog/albums/{id}/artwork/file`** — 复刻 `streamTrack` 的双重认证（Authorization Bearer 头，或 `?exp=&sig=` HMAC 签名查询串——`<img>` 标签和 `<audio>` 标签一样无法自定义请求头），不套 `requireViewerAuth`，因为签名 URL 路径本身就是头部认证的**替代**而非附加。`GET .../artwork` 的响应形状不变（仍是 `{url, expiresIn}`），只是 `url` 现在在本地/NFS/SMB 后端上是一个**带签名的相对路径**（与 `streamUrl` 完全同构），而非直接抛错。
- **加了进程内缓存，字节端点也带 `ETag`/`Cache-Control`** — 解析封面意味着打开文件、解析标签、可能还要扫目录，代价不该在每次请求都掏一遍；`artworkCache`（`internal/httpapi/artwork_cache.go`）按专辑 ID 做 10 分钟 TTL 缓存，连"确认没有封面"的负结果也一并缓存，避免反复对同一张空专辑做无谓的文件系统访问。只读挂载排除了落盘缓存，所以只能是进程内、重启即失效的形态。字节响应用内容哈希算 `ETag`，`http.ServeContent` 原生处理 `If-None-Match` 生出 304。
- **客户端半边：为什么这是同一个版本的两半，缺一不可** — `services/mobile/lib/src/catalog/artwork_provider.dart` 原来把 `resp.data?['url']` 原样交给图片组件。服务端一旦对本地后端开始返回**相对**签名路径，这条路径会精确复现 v5.37.1 播放那次的 `-1002 unsupported URL`（没有 scheme、没有 host）——只是这次是封面图加载失败，不是播放失败。**只发服务端半边会把"无封面占位图"换成"图片加载失败"，比现状更糟**。v5.37.1 已经在 `player_notifier.dart` 解过完全同构的问题（`absolutePlaybackUrl`，基于 `Uri.parse(base).resolve(url)`），本次把它泛化搬到 `lib/src/shared/absolute_url.dart`（改名 `absoluteApiUrl`，doc comment 扩写为同时覆盖播放流与专辑封面两种场景），`player_notifier.dart` 保留 `absolutePlaybackUrl` 作为薄别名转调新函数（`test/playback_url_test.dart` 零改动仍然全绿，这本身就是"泛化没有破坏原有行为"的一次证明）。`artwork_provider.dart` 改用 `absoluteApiUrl(url, resp.requestOptions.baseUrl)`——刻意不引入 `baseUrlProvider`：那条 provider 链最终要摸 `SharedPreferences.getInstance()`/`flutter_secure_storage`，而既有 4 条 `artwork_url_provider_test.dart` 用例从未在测试里初始化/mock 这两个插件（全靠覆盖 `dioProvider` 绕开），贸然接入会让这批已有测试开始触碰平台通道、有炸掉的风险；`resp.requestOptions.baseUrl` 是 dio 拦截器实际用于**这一次请求**的 base（由 `dioProvider` 的认证拦截器逐请求写入 `RequestOptions`，而非改写共享的 `Dio.options`），语义上比另开一条 provider 读取更准确，也完全不引入新依赖。
- **守卫测试及证伪** — 最关键的一条：`TestGetAlbumArtwork_LocalBackend_EmbeddedPicture_Success`（`internal/httpapi/handler_artwork_fallback_test.go`）注册一个真实 `type: "local"` 后端（临时目录做 rootPath）、写入一个手工拼出的带内嵌图 FLAC 文件、专辑 `ArtworkMediaObjectID` 显式关联（复现"封面正确关联但后端是 local"这个 v5.37.1 记录场景）。**证伪**：把 `getAlbumArtwork` 函数体临时换回修复前的原样代码（其余新基建——`artworkCache`/`resolveAlbumArtworkCached`/新端点——保持不动），重跑这一条，精确复现 `503 {"error":{"code":"presign_failed","message":"failed to generate artwork URL"}}`；恢复后转绿。另外四组证伪：sibling 文件回退（临时短路 `FindSibling` 调用，结果变回 404 而非 200）、诚实 404（临时让 `resolveAlbumArtwork` 恒定返回一个伪造图片，证明"服务端绝不合成占位图"这条承诺真的被测试盯着——结果变回 200 而非期望的 404）、缓存生效（临时跳过缓存读取路径，文件被覆写后第二次请求返回了新内容而非应有的旧内容）、以及 `internal/artwork` 包级别对 `ExtractEmbedded`/`FindSibling` 各自的独立证伪。客户端一侧：把 `absoluteApiUrl` 临时退化成恒等函数，新增的 `test/absolute_url_test.dart`（6 条）与 `test/artwork_url_provider_test.dart` 新增的相对路径用例、以及**既有的** `test/playback_url_test.dart`（因为 `absolutePlaybackUrl` 是薄别名）一并精确失败，证明播放与封面两条路径确实共享同一份泛化实现，不是各自为政。全部证伪均已恢复并重新确认全绿。
- **验证** — Go 侧（`services/api`）：`gofmt -l .` 无输出；`go vet ./...` 通过；`go build ./...` 通过；`go test ./...` **822 passed**（v5.38.1 基线 800 + 新增 22：`internal/artwork` 9 条、`internal/storage` 新增 `LocalPath` 3 条、`internal/streamsign` 新增 1 条子测试、`internal/httpapi` 新增 9 条，零失败；另确认 `internal/catalog` 的 `TestUpdateAlbumChangesTitle` 是与本次改动无关的既有间歇性时钟精度 flaky——同一份未改动代码单独重跑 5 次里失败 1 次，本次所有改动过的包连续多轮零 flaky）；`go test -race ./...` 全部包 `ok`。Flutter 侧（`services/mobile`）：`flutter analyze --no-fatal-infos` 0 error/0 warning，仅 `player_state_reporter.dart:21` 一条既有 info（未触碰该文件）；`flutter test` **440 passed**（v5.38.1 基线 433 + 新增 7）。`dart format` 只对本次改动的 5 个手写文件执行（`lib/src/shared/absolute_url.dart`、`lib/src/player/player_notifier.dart`、`lib/src/catalog/artwork_provider.dart`、`test/absolute_url_test.dart`、`test/artwork_url_provider_test.dart`），未对目录跑。OpenAPI：`packages/api-contract/openapi/storage-admin.v1.json` 新增 `/api/v1/catalog/albums/{id}/artwork/file` 路径，`info.version` 从 `5.35.0` 对齐到 `5.39.0`；`python3 -m json.tool` 校验通过。生产服务器本身无法触达也不会尝试连接，全部验证依赖本地夹具。
- **未做 / 已知限制** — 只覆盖 `dhowden/tag` 支持的格式（FLAC/OGG/MP4/ID3v2/ID3v1/DSF），WAV 等它不支持的格式会静默落到 sibling 文件回退，找不到就诚实 404。sibling 文件候选名单固定为 `cover`/`folder`/`front`/`album` 四个，不支持用户自定义扩展。`ArtworkMediaObjectID`+本地后端组合下，就算手工 PATCH 关联了一个真实图片媒体对象，也**不会**被直接读取/served——本版本的回退分支统一走"从曲目重新解析"，不单独为这个组合再实现一条"读取已关联媒体对象字节"的路径；这在当前生产数据下是等价的（`ArtworkMediaObjectID` 目前恒为空），但如果未来有人在本地后端上手工关联了一个专门的封面媒体对象，这条关联会被忽略，值得记录。iOS ATS 拦播放（v5.36.0 记录）仍未修。
...
- **fix: 构建期配置契约——生产构建缺失 `NEXT_PUBLIC_API_BASE_URL` 时静默烤入 `localhost`（2026-08-11 生产事故根因）** — 生产环境（Unraid）出现登录失败事故：用户直连 web 容器 `IP:3000` 绕过 nginx 网关，浏览器侧 `services/web/lib/api/client.ts` 的 `baseUrl` 在 `typeof window !== "undefined"` 时是 `""`（同源相对路径），`/api/v1/auth/login` 落到 Next.js 自己的 `rewrites()`。`next.config.ts` 的 `rewrites()` 原写法是 `process.env.NEXT_PUBLIC_API_BASE_URL ?? "http://localhost:8080"`，而 `services/web/Dockerfile`/`services/admin/Dockerfile` 均缺少 `ARG NEXT_PUBLIC_API_BASE_URL`——`.github/workflows/docker.yml` 传的 `--build-arg NEXT_PUBLIC_API_BASE_URL=http://api:8080` 被 BuildKit 判定为「未声明、未消费」直接丢弃（非致命警告，构建照常成功），`next build` 在容器里拿到的变量是空的，兜底值 `http://localhost:8080` 被永久烤进产物 `.next/routes-manifest.json`。运行时再设 `NEXT_PUBLIC_API_BASE_URL` 环境变量完全无效——`next start`/standalone `server.js` 只读预编译的 manifest，不会重新执行 `next.config.ts` 的 `rewrites()`；`docker-compose.prod.yml` 里的同名运行时变量因此成了从未生效的摆设。web 容器里没有任何东西监听 8080，登录请求 `ECONNREFUSED`，前端只看到裸的 500，本仓库反复出现的「静默失败」模式又一次命中。
- **修复** — 两个 Dockerfile 的 builder stage（`RUN npm run build` 之前）各加 `ARG NEXT_PUBLIC_API_BASE_URL` + `ENV NEXT_PUBLIC_API_BASE_URL=${NEXT_PUBLIC_API_BASE_URL}`，让 CI 已经在传的 build-arg 真正流入构建环境。`next.config.ts`（web/admin 各一份）改造成 phase 函数式导出 `(phase) => NextConfig`，缺变量时在 `phase === PHASE_PRODUCTION_BUILD` 下直接 `throw`，绝不再静默回落；开发环境兜底值原样保留。判断条件特意选 Next.js 自带的 `phase`（`next/constants`）而不是 `NODE_ENV`：核实 Next.js CLI 源码（`node_modules/next/dist/bin/next`）确认 `next start` 同样把 `NODE_ENV` 默认置为 `"production"`（`defaultEnv = commandName === "dev" ? "development" : "production"`），`NODE_ENV` 根本区分不出"正在构建"和"正在跑一个已构建好的服务"；而 `phase` 由 Next.js 在配置加载点传入，`next build` 走 `PHASE_PRODUCTION_BUILD`（`next/dist/build/index.js`），`next start` 走 `PHASE_PRODUCTION_SERVER`（`next/dist/server/next.js`），二者精确不重叠——用 `phase` 意味着缺变量只可能炸构建本身，不可能牵连一个已经在跑的生产服务。排查 `.github/workflows/build.yml` 时顺带发现 `admin-e2e` job 的「Build admin」步骤（`npx next build`）本来就没设该变量，一直靠这次要拆掉的静默兜底才跑得通，是同一 bug 模式在 CI 里的第三处实例；不补上会被本次的 throw 直接炸穿 CI，属于本次改动的直接连带后果，已一并补上 `NEXT_PUBLIC_API_BASE_URL: http://localhost:8080`（与同 workflow 其余 4 处保持一致）。
- **守卫测试（双层）及证伪** — 单测层：`services/web/next.config.test.ts` 直接调用 `next.config.ts` 导出的函数，覆盖「缺变量炸构建」「变量为空字符串炸构建」「变量正确不炸且不含 localhost」「dev phase 兜底不受影响」四种场景，用 `vi.stubEnv`/`vi.unstubAllEnvs` 操作 `process.env`（而非 `delete`/直接赋值 `undefined`——已实测确认 `process.env.X = undefined` 会把值字符串化成字面量 `"undefined"` 而非真正删除键，`vi.stubEnv(name, undefined)` 内部才是真 `delete`），随常规 `npm test` 秒级跑完。构建产物层：`scripts/verify-production-build-requires-api-base-url.mjs`（web/admin 各一份）实际执行两次真实 `npm run build`——场景 A 不设变量，必须整个构建失败且报错信息点名变量；场景 B 设成非 localhost 的真实值（`http://api:8080`），必须构建成功且 `.next/routes-manifest.json` 的 rewrite destination 精确带出该值、不含 `localhost`——直接断言任务原话点名的构建产物，已接入 `.github/workflows/build.yml` 的 web/admin 两个 job（各自「Build」步骤后新增一步）。admin 没有 Vitest/单测基建（只有 Playwright e2e，此前没有任何 `*.test.ts` 先例），因此没有为它凑一份不会被任何脚本执行的形式化单测，只保留构建产物层——两端最终都是真实会被执行到的检查。证伪：用 `git stash` 只回退 `next.config.ts` 到修复前版本跑守卫脚本，web/admin 均在场景 A 精确失败（`FAIL: production build succeeded WITHOUT NEXT_PUBLIC_API_BASE_URL and silently baked "localhost" into .next/routes-manifest.json`，退出码 1）；`git stash pop` 恢复后重跑两端均 `ALL GUARDS HELD`（退出码 0），证明守卫不是摆设。
- **fix: `local` 存储后端健康探测在只读挂载上必然误报 unhealthy** — `services/api/internal/storage/probe.go` 的 `FilesystemProber.Probe` 原先无条件对所有挂载类文件系统后端（local/nfs/smb/distributed 的 mounted-filesystem adapter）写一个探针文件来判定可达性；而音乐服务器把媒体库以 `:ro` 只读挂载进容器是常态（`docker-compose.prod.yml` 里 `# - /mnt/music:/mnt/music:ro` 就是现成的例子），写探针必然因权限被拒而报 unhealthy，与后端是否真正可达无关。`LocalConfig` 新增 `ReadOnly bool` 字段（零值 `false`，对既有后端配置完全无感），`Probe` 在 `backend.Type == BackendTypeLocal && Config.Local.ReadOnly` 时改走新增的 `probeReadableDirectory`——只做 `os.Open` + `Readdirnames(1)`，`io.EOF`（空目录）视为健康而非失败；未标记 `ReadOnly` 的 local 后端（含所有历史数据，零值即原语义）与 nfs/smb/distributed 后端完全不变，仍走原写探针路径，「可写后端仍应能探测出能写」的语义不受影响。范围收在 `local` 类型（任务原话与事故场景 `/media/music:ro` 均指向本地 bind mount）；nfs/smb 挂载理论上有同样的只读诉求，但配置结构各自独立、不在本次任务范围内，如实记录留作后续。OpenAPI `LocalConfig` schema 同步加 `readOnly`（可选，不进 `required`），`info.version` 从 `5.4.0` 对齐到本次发布号 `5.35.0`（这份契约版本历史上并非每次发布必动，只在 schema 真变化时才跳且不严格跟随发布号——此次选择直接对齐更便于追溯，理由详见 `.plan`）。
- **验证** — Go 侧（`services/api` 内）：`gofmt -l .` 无输出；`go vet ./...` 通过；`go build ./...` 通过；`go test ./...` **749 passed**（v5.34.1 基线 745 + 新增 4 条 `probe_test.go` 守卫测试，零失败）；`go test -race ./...` 全部包 `ok`；OpenAPI JSON 经 `python3 -m json.tool` 校验通过。只读探针守卫的证伪：`git stash` 只回退 `probe.go`（保留新增的 `ReadOnly` 字段，仍可编译），`TestFilesystemProberReadOnlyLocalSkipsWriteProbe` 精确失败并报出探针文件创建 `permission denied`，`git stash pop` 恢复后转为全绿。Web/Admin 侧：`npm run lint`（Biome）、`npm run type-check`（`tsc --noEmit`）均 0 错误；Web `npx vitest run` **278 passed**；两端 `scripts/verify-production-build-requires-api-base-url.mjs` 均 `ALL GUARDS HELD`。本机确认无 `docker` 命令（`which docker` 未找到）且任务明确要求不动 docker，两个 Dockerfile 的 `ARG`/`ENV` 改动仅做人工审阅、未做本地 `docker build` 验证，留给 CI 的 `Docker` workflow 确认。

### v5.34.1 - 2026-08-11

- **fix: Meilisearch 部署了容器却从未接线（v5.34.0 汇报里报告的第三个同类 bug）** — `docker-compose.prod.yml` 定义并跑起了 `meilisearch` 服务，但 `api` 服务的 `environment` 段里一直没有 `MEILI_HOST`/`MEILI_SEARCH_KEY`。源码 `cmd/server/main.go:114` 是 `if meiliHost := os.Getenv("MEILI_HOST"); meiliHost != "" { ... }`——变量为空就整段跳过，且跳过分支**没有任何日志**（只有启用成功才打 `meilisearch enabled at %s`），比 v5.34.0 的 Bug 1/Bug 2 更隐蔽：容器正常运行、搜索功能正常可用（静默退回 PostgreSQL 全文检索），完全没有错误或提示指向「Meilisearch 其实没被用上」。同一台 Unraid 主机上跑着的 v5.4.0 部署确认设了这两个变量——这是三个 bug 里第二次出现「实际部署比仓库 compose 正确」，`docker-compose.prod.yml` 大概率从未被真正跑通过。
- **修复** — `MEILI_HOST` 接为 compose 内部固定地址 `http://meilisearch:7700`（写法比照 `INORI_DATABASE_URL` 用 `postgres` 服务名的方式，不做成可留空——这份 compose 里 meilisearch 容器本来就无条件启动，没有理由不接）；`MEILI_SEARCH_KEY` 复用与 `meilisearch` 服务 `MEILI_MASTER_KEY` **完全相同**的 `${MEILI_SEARCH_KEY:?MEILI_SEARCH_KEY required}` 插值表达式，确保两边永远拿到同一个 `.env` 值。顺带补上 compose 头部注释块里漏掉的两个 `:?required` 变量说明——`INORI_STREAM_SIGNING_KEY`（v5.34.0 就已是必填，头部注释一直没提）和 `MEILI_SEARCH_KEY`（现在被两个服务同时依赖，更需要在头部注明）。
- **Meilisearch master key 与 search key 语义核实** — 查证 Meilisearch 官方文档确认：master key 拥有全部权限（含 key 管理），是所有其他 key 的超集；自动生成的 Default Search API Key 仅有 `search` 权限，**不能**创建索引、更新可搜索属性或增删文档。而本仓库 `internal/search/meilisearch.go` 的 `NewMeilisearch` 在启动时会调用 `CreateIndex`/`UpdateSearchableAttributes`（`ensureIndexes()`），运行中还会 `AddDocuments`/`DeleteAllDocumentsWithContext`——这些都是管理级操作，一个真正的「search key」权限不够，会导致索引创建/属性更新静默失败（`CreateIndex` 的错误被当前代码整体忽略）。结论：`api` 服务的 `MEILI_SEARCH_KEY` **必须**与 meilisearch 服务的 master key 是同一个值，这不是权宜之计而是当前实现的硬性要求；代价是 Go 后端服务器持有 master key 而非最小权限 key，但这个 key 从未流向任何客户端/浏览器，始终留在 compose 内部服务间通信中，且要拿到一个范围合适的自定义 key 需要在 Meilisearch 启动后额外调用一次 `/keys`（用 master key 认证）来置备——docker-compose 的静态环境变量插值做不到这一步，除非引入额外的置备脚本，超出本 phase 范围。
- **守卫：`compose_env_test.go` 新增反方向检查** — v5.34.0 的 `TestComposeAPIEnvVarsMatchSource` 只查「compose 写了、源码不读」的单向漂移，刻意如此避免对 `INORI_CORS_ORIGINS`/`INORI_SESSION_TTL` 这类留空也能正常工作的可选变量误报。本次新增 `TestComposeAPIEnvVarsIncludeSilentlyDegradingVars`，用一份手工维护的 `silentlyDegradingEnvVars` 清单（而非自动扫描全部 `os.Getenv`）登记「不设即静默降级」的变量，断言清单里每一项都必须出现在 compose 的 `api.environment` 里；清单额外做了自校验，登记的名字如果在 `main.go` 里已经不存在对应的 `os.Getenv` 调用会直接报错，防止清单本身腐化。为让两条测试共享同一套 Go AST / YAML 解析逻辑，把原先写死的 `INORI_` 前缀过滤抽成 `hasTrackedPrefix`（`INORI_`/`MEILI_` 两个前缀），两条测试都改用它，互不影响。
- **认真扫过 v5.34.0 汇报列出的全部 13 个 `INORI_*`/`MEILI_*` 变量，最终只登记 `MEILI_HOST`/`MEILI_SEARCH_KEY` 两项** — 判断标准：变量不设时是否存在「compose 已经为它配了专属基础设施（跑着一个容器），源码却完全没打日志就悄悄绕过」这种「意图与现实脱节」的信号。`INORI_ADMIN_TOKEN`/`INORI_STREAM_SIGNING_KEY` 已经是 compose 里的 `:?required`，比清单机制更强，不必重复登记；`INORI_DATABASE_URL` 不设时会在四个服务构造点各打一条明确日志，不算「静默」；`INORI_INITIAL_ADMIN_USER`/`PASSWORD`、`INORI_CORS_ORIGINS`、`INORI_SESSION_TTL` 都是 compose 注释里已经写明的、有完整兜底行为的正经可选项（任务原文明确点名后两者不该被误纳入）。`INORI_STORAGE_REFRESH_INTERVAL` 是本次判断上最接近边界的一个——不设时同样零日志，但它只是关掉一个进程内定时器，没有配套的独立容器/基础设施被浪费，且 compose 注释早已把它写成刻意的可选项，判定不登记。另发现 `INORI_STORAGE_REPOSITORY_FILE`/`INORI_MEDIA_OBJECT_REPOSITORY_FILE` 在当前 compose 拓扑下因为 `INORI_DATABASE_URL` 恒非空而永远走不到读取它们的分支（纯粹的死配置，不是「未设置会怎样」的问题），这是另一种性质的小瑕疵，本 phase 不动，如实记录。另外发现仓库根目录下还有一个 `docker-compose.v484-test.yml`（v4.8.4 遗留的一次性测试用 overlay），仍是修复前的 `INORI_DB_DSN`/`INORI_AUTH_SECRET` 且同样没接 Meilisearch——任务范围明确限定在 `docker-compose.prod.yml`，这份陈旧文件不在本次修改范围内，如实记录以免被误认为遗漏。
- **验证** — `gofmt -l services/api` 无输出；`go vet ./...` 通过；`go test ./...` **745 passed**（v5.34.0 基线 744 + 新增 1 条守卫测试，零失败）；`go test -race ./...`（`-count=1` 强制不吃缓存）全部包 `ok`；空测试验证——临时从 compose 里删掉 `MEILI_HOST` 一行后 `go test -count=1` 精确报出 `TestComposeAPIEnvVarsIncludeSilentlyDegradingVars` 失败并给出诊断信息（含变量名与「不设会怎样」的说明），随后已恢复原文件并确认 `diff` 与备份一致；OpenAPI 无 schema 变更，`info.version` 未改动。

### v5.34.0 - 2026-08-10

- **fix: 部署前排查发现的两个服务端真 bug——迁移未接线 + compose 环境变量契约错误** — 计划把服务端部署到一台 Unraid 主机上，动手前先审查代码，发现两个会让部署静默出错的真 bug，均已在源码里逐一核实，不是猜测。修复后为两类问题各补一条静态架构测试守卫，防止同类问题第三次发生。
- **Bug 1：两个 Postgres migration 从未被调用** — `cmd/server/main.go` 构造 `playerstatepg.NewRepository(pool)`（第 141 行）与 `searchhistorypg.NewRepository(pool)`（第 151 行）时，从未调用过两个包各自定义的 `Migrate`。全仓库 grep `.Migrate(ctx` 此前只命中 `pgstore.Migrate` 与 `userplaylistpg.Migrate` 两处调用点；`NewRepository` 不做惰性建表，只是存了 pool。后果：`user_player_state` 与 `user_search_history` 两张表在真实 Postgres 上永远不会被创建——这正是 v5.4.0「跨设备续播 + 搜索历史同步」整个特性能落地的前提。四个版本没被发现的原因：`internal/playerstate/postgres/` 与 `internal/searchhistory/postgres/` 两个包一个测试文件都没有，单元测试全部走内存实现，Postgres 路径从未被真正执行过；同目录下的 `internal/userplaylist/postgres/` 同样零测试，但它的 `Migrate` 恰好被调了，说明这是遗漏而非设计。修复：在 `main()` 对应的 `if pool != nil` 分支内补上 `Migrate` 调用，错误处理照 `userplaylistpg.Migrate` 现有写法——`log.Printf` 记录但不中断启动，与既有风格保持一致，不额外发明新的错误处理策略。
- **Bug 2：`docker-compose.prod.yml` 的环境变量契约与源码不符** — compose 里 `api` 服务写的是 `INORI_DB_DSN`，源码读的却是 `INORI_DATABASE_URL`（`main.go:267`），`INORI_DB_DSN` 在 Go 源码里零命中。照这份 compose 部署，`openDatabasePool` 读不到 DSN 会直接返回 `nil, nil`，API 走进「`INORI_DATABASE_URL is not set`」分支，用内存态运行认证/会话——容器正常启动、健康检查正常通过，一重启用户和会话全部丢失，是典型的静默失败。compose 里同时还有一个死变量 `INORI_AUTH_SECRET`（标成 required），源码里同样零命中。逐一核对源码里全部 `INORI_[A-Z_]+`/`MEILI_[A-Z_]+` 字符串字面量后修复：`INORI_DB_DSN` 改名为 `INORI_DATABASE_URL`；删除 `INORI_AUTH_SECRET`（含顶部注释块里的提及）；补上源码支持但 compose 未提及的四组变量——`INORI_INITIAL_ADMIN_USER`/`INORI_INITIAL_ADMIN_PASSWORD`（可选，留空即跳过初始管理员创建而不是报错；两者都设置才能在全新数据库上完成自助登录）、`INORI_CORS_ORIGINS`（可选，留空是宽松模式，即反射任意来源）、`INORI_SESSION_TTL`（默认值 `24h` 直接取自源码 `sessionTTL()` 的硬编码默认，非臆测）、`INORI_STORAGE_REFRESH_INTERVAL`（可选，留空即关闭周期刷新调度器，与源码默认行为一致）。四组变量都通过 compose 的 `${VAR:-default}` 插值语法接入 `.env`，给出的默认值要么留空、要么是核实过的源码真实默认值，没有编造任何取值；`docker-compose.prod.yml` 其余部分（这是一份通用参考部署，不是 Unraid 主机的具体实例）未改动。
- **守卫测试一：迁移接线覆盖率** — 新增 `services/api/cmd/server/migrate_coverage_test.go`（`TestPostgresMigrateFuncsAreCalledInMain`），用标准库 `go/parser`+`go/ast` 静态扫描 `internal/*/postgres/*.go`，找出所有导出的包级 `Migrate` 函数（方法不算，只认能直接调用的包函数——与本仓库现有四个 `Migrate` 的写法一致），再解析 `main.go` 的 import 与调用表达式树，断言每一个导出 `Migrate` 的包都在 `main.go` 里被导入且以 `<alias>.Migrate(...)` 的形式实际调用过。纯静态检查，不连数据库，不引入新依赖。用修复前的 `main.go` + 新测试文件验证过一遍：精确报出 `playerstate/postgres` 与 `searchhistory/postgres` 两个包，证明测试确实能抓住这类 bug，不是摆设。
- **守卫测试二：compose 环境变量契约** — 新增 `services/api/cmd/server/compose_env_test.go`（`TestComposeAPIEnvVarsMatchSource`），交叉核对 `docker-compose.prod.yml` 的 `services.api.environment` 与 `cmd/server/main.go`（Dockerfile 里实际构建进镜像的入口）中 `os.Getenv("INORI_...")` 读取的名字集合，compose 里出现源码不读的名字就失败（只做单向检查，不反向要求源码支持的每个变量都必须出现在 compose 里，否则会对 E2E 测试专用变量等合理缺省产生误报）。YAML 解析用 `gopkg.in/yaml.v3`——该库本来就是 `testcontainers-go` 拉进来的间接依赖，`go.sum` 里已有完整哈希，直接 import 后跑 `go mod tidy` 把它从 indirect 提升为 direct，没有引入任何新的外部依赖（`go mod tidy` 顺带修正了此前遗漏的 `go-pinyin` indirect→direct 状态，属存量 go.mod 卫生问题，非本次引入，一并处理）。仓库根目录的 `docker-compose.prod.yml` 在 `services/api` 这个 Go module 之外，测试用向上查找 `go.work` 定位仓库根，不依赖写死的相对路径层数。同样用修复前的 compose 文件验证过：精确报出 `INORI_AUTH_SECRET, INORI_DB_DSN` 两个死变量。
- **migration 可运行测试——评估结论：值得做，但留给下一个 phase 决定** — `playerstate/postgres`、`searchhistory/postgres`、`userplaylist/postgres` 三个包全部没有测试文件；其中 `playerstate`、`searchhistory` 连不需要数据库的 service 层单测都没有（`userplaylist` 的 service 层有测试，只是 postgres 子包没有）。调研发现仓库其实已经具备所需基建：`testcontainers-go/modules/postgres` 已是 `go.mod` 里的直接依赖，`catalog/postgres`、`storage/postgres`、`history/postgres` 三个包已有 `//go:build integration` 标记的 testcontainers 集成测试作为先例——但 `Build` workflow 的 `go test` 步骤没有传 `-tags=integration`，这类测试目前完全不在 CI 里跑，只是本地手动可用的安全网。既然基建已经存在，给 `playerstate/postgres`/`searchhistory/postgres`（乃至 `userplaylist/postgres`）补同样模式的集成测试不需要引入任何新依赖，是本次评估后认为值得做但没有顺手做的候选项，留给用户决定是否排期、以及要不要顺带把这类测试接进 CI。
- The phase output is version-tracked and verified locally：`gofmt -l services/api` 无输出，`go vet ./...` 通过，`go test ./...` **744 passed**（v5.33.0 基线 742 + 新增 2 条守卫测试，零失败），`go test -race ./...` 全部包 `ok`，OpenAPI JSON（`storage-admin.v1.json`）经 `python3 -m json.tool` 校验通过、`info.version` 未改动（本次无 schema 变更）。`docker build` 冒烟测试未在本地验证（本机无 docker 命令），留给 CI 确认。

### v5.33.0 - 2026-08-10

- **feat: 侧栏菜单重做（EchoMusic 大平台骨架）+ 队列抽屉 + 三个实机反馈 bug 修复** — 用户 v5.32.0 实机验收「基本满足要求」，转场与进度条通过，同时报了三个 bug 并追加了播放条留白要求；本 phase 先修这三条，再做原计划里顺延过来的侧栏菜单重做。方向依据是 2026-08-10 定下的「统一设计语言」战略（见 `smooth-toasting-manatee.md`「批次三」）：不再追单一 OS 原生保真，改用 EchoMusic 那套自洽的跨平台语言，唯一例外是 macOS 红绿灯保持系统绘制。
- **Bug 1：播放条时间标签换行** — 用户截图里 `0:43` 渲染成两行 `0:4`/`3`、`3:05` 渲染成 `3:0`/`5`，最后一个字符掉到第二行。根因是三个问题叠加：`_timeLabelWidth` 里的 `TextPainter` 没有接环境 `MediaQuery.textScalerOf(context)`（渲染按放大后的字号、测量按 1.0），量出的 `painter.width` 是小数却原样当作 `SizedBox.width`（零余量，段落布局一舍入最后一个字形就挤不下），而那个 `Text` 又没有 `maxLines: 1`/`softWrap: false` 兜底。三条全部修正：测量传入与渲染一致的 `textScaler`、宽度 `ceilToDouble()` 后加余量、`Text` 显式单行不换行。测试在放大的 `textScaler` 下断言实际渲染行数仍为 1，而非仅断言 widget 存在；v5.32.0 那条「位数变化时轨道宽度不变」的断言保留未削弱。
- **Bug 2：队列按钮打开的是播放页而非队列** — 这是 v5.30.5 明确指示的临时做法（当时定为「没有从主界面打开队列的通路就先跳播放页，不要为此新造队列 UI」），本 phase 补正。新增 `lib/src/player/queue_drawer.dart` + `queue_drawer_provider.dart`：点击从右侧滑出抽屉覆盖在内容区上，不离开当前路由，点击外部或 Esc 关闭。队列列表本身从 `full_player_screen.dart` 的私有 `_QueueList` 提取为公开的 `lib/src/player/queue_list.dart`，全屏播放页的侧栏面板（两处调用）与新抽屉**共用同一份实现**——两套 UI 一定会在重排/删除/跳播这些行为上分叉。
- **Bug 3：播放条留白四边不等** — 用户原话「控制条完全可以参考 EchoMusic 这样的上下左右留白等宽，而不是现在我们这种基本贴边」。此前外边距是 `EdgeInsets.fromLTRB(8, 0, 8, 8)`——左右下三边都是 8，唯独顶边是 0，于是播放条顶边直接贴着上方内容区。改为单个具名常量 `_outerMargin = 8.0` 经 `EdgeInsets.all` 应用：**用一个常量驱动四边才是真正的修复，而不是把那个 0 改成 8**——四个独立字面量正是让其中三个达成一致、第四个掉队的原因，再补一个临时字面量只会重复同一个错误。测试直接量封面到左边框与到上边框的距离是否相等（允许 1px 误差），而不是断言常量取值，否则改了常量测试照样过。
- **侧栏菜单重做（本 phase 大头，仅桌面 `_DesktopSidebar` ≥1200dp）** — 参照实测自 EchoMusic 源码 `src/renderer/layouts/Sidebar.vue`（已拉取核对，非凭印象）。**账号卡片**：把原先裸的一行重做成带浅填充的圆角卡片（头像 + 主行 + 次行 + 右侧齿轮进设置），游客态主行「未登录」、次行「点击登录账号」，`exitGuestMode()` 行为原样保留只改外观。**可折叠分组**：`发现音乐` / `我的乐库` 加折叠能力，状态经新增的 `sidebar_group_collapse_provider.dart` 持久化（照 `coverFlowModeProvider` 的最小写法：SharedPreferences + Notifier，不造设置子系统）。**分组内容按真实能力映射**：`发现音乐` 下「探索发现」用现有 catalog 接口做出真内容（最近添加 / 随机推荐两个区块，新增 `lib/src/discover/explore_screen.dart`），为此给 `CatalogRepository.listAlbums` 补了 `sortBy`/`sortOrder` 透传——`createdAt` 是服务端 `catalog/types.go` 里 `AlbumSortByCreatedAt` 已支持的真实取值，先核对过再加，不是推测出来的参数；「为您推荐」（`for_you_screen.dart`）做引导性空状态，文案说明依赖后续版本。**私人 FM / 我的云盘刻意不做**：它们强依赖平台侧能力，本项目服务端完全没有对应物，摆出来就是点进去永远同一句话的死入口——这一条在规划阶段已向用户说明。**歌单分栏**：新增 `sidebar_playlists_section.dart`，`自建歌单`（`user_playlist_*` 本账户歌单）| `收藏歌单`（服务端策展 catalog playlists）两个 tab + 排序/刷新 + 「查看全部」入口，未登录时空状态引导登录。**底部两个入口**：「下载」接既有离线下载能力（指向设置页 Offline Library，不新造下载系统）；「装置」（输出设备选择）**按能力门控**，读 `playbackCapabilitiesProvider.select((c) => c.outputDeviceSelection)`——当前 just_audio 引擎下该位为 false，所以这个入口现在完全不渲染（而非渲染成禁用态），等后续 media_kit / 自研引擎上报 true 时它自己出现，是能力驱动而非硬编码隐藏。
- **范围限制与回归守卫** — `_TabletLayout` 的 NavigationRail 与 `_MobileLayout` 的 NavigationBar 保持原样，扁平 `_navItems` 索引机制不变；「移动端底栏仍是 6 个目的地」这条既有断言作为回归守卫保留（重构中间态曾一度打破过它，说明菜单重做漏进了移动端，已修正）。「Playlists 可从侧栏到达」这个**行为**断言在结构改变后指向新的歌单分栏路径而非被删除。v5.31.0 那条「渐变层不拦截点击」的回归守卫同样保留，仅调整选择器。侧栏那个 `final index = flatIndex;` 的闭包快照在折叠分组引入后保留——它是 v5.22.0 「所有导航项都失效」那个 bug 的防线。齐平化侧栏靠 `Material(type: MaterialType.transparency)` 提供 `ListTile` 所需的 Material 祖先，重构中保持未丢。
- **l10n** — 新增 `forYou`/`forYouComingSoon`/`sidebarExplore`/`recentlyAdded`/`randomPicks`/`createdPlaylists`/`collectedPlaylists`/`sortAlphabetically`/`refresh`/`viewAll`/`noCreatedPlaylistsYet`/`noCollectedPlaylistsYet`/`notSignedIn`/`downloads`/`outputDevice` 共 15 个 key，en/ja/zh 三语齐全。
- The phase output is version-tracked and verified locally：`flutter analyze --no-fatal-infos` 0 error/warning（仅 `player_state_reporter.dart:21` 一条既有 info，未触碰该文件），`flutter test` **404 passed**（v5.32.0 基线 383 + 新增 21）。纯客户端改动，无服务端 schema 变更，故不同步 OpenAPI `info.version`。

### v5.32.0 - 2026-08-10

- **feat: 播放页 Apple Music 式转场（上滑打开/下滑手势关闭）+ 打开卡顿修复 + 进度条 hover 收缩真 bug 修复 + 进度条比例重调 + 封面 hover 放大** — 用户 v5.31.0 实机验收「这下看起来不违和了」，侧栏齐平化通过，本批处理剩余两条交互手感反馈：播放页转场太廉价（默认 `FadeUpwardsPageTransitionsBuilder` 只位移 25% + 淡入）、打开时会卡顿；顺带处理调研中确认的进度条 hover 收缩真 bug、进度条比例失调、封面缺 hover 提示。原计划（`smooth-toasting-manatee.md`「批次三」）把转场排在 v5.33.0、菜单重做排 v5.32.0，用户新反馈把转场提前，菜单重做顺延一版——这是本 phase 与既定顺序的唯一差异。
- **A. 播放页转场重做**：`lib/src/shared/router.dart` 的 `/player` 路由从 `builder:` 改 `pageBuilder:` + `CustomTransitionPage`（`opaque: false`，保留下层 shell 可见供渐暗遮罩衬底），新文件 `lib/src/player/player_transition.dart` 承载实际机制。整屏从底部升起盖住全窗，`Curves.easeOutCubic` 直接套用在路由 `animation` 原始值上（不用 `CurvedAnimation` 的 `reverseCurve`——推导证实同一条曲线不分方向应用時，进场天然读出减速形状、退场天然读出对称的加速形状，`CurvedAnimation.value` 源码确认 `reverseCurve` 留空就是复用同一条曲线，没有必要多引入一个对象）；时长 320ms，取 300–360ms 区间中点，参照本仓库已有的 260ms（`positionTweenDuration`）与 300ms（歌词自动滚动）两个时长定位——比两者都慢一档，因为这是"整窗内容切换"级别的动作，但这也是高频交互（一场会话开关很多次），不宜比 300ms 区间上限更慢。**下滑手势关闭**：`_PlayerTransitionShell` 自建一个独立 `AnimationController`（值域即"多开"，1=全开，0=全关）而非直接操作路由自身的 `AnimationController`——`TransitionRoute.controller` 标了 `@protected`，Cupertino 内建的边缘返回手势能碰它是因为那段代码本身就混入在路由类里，本仓库的手柄只是路由树里一个普通后代 widget，碰它会触发 `invalid_use_of_protected_member`，而本仓库的 `flutter analyze` 门槛是新增 warning 记为失败。拖动跟手（直接赋值，不套曲线），松手按位移（>30% 屏高）或速度（>700px/s，复用 Flutter `Dismissible` 自己的 `_kMinFlingVelocity` 常量而非另起一个阈值）二选一决定关闭还是回弹；关闭动画播完才真正 `Navigator.maybePop()`，避免路由自身的 reverse 过渡半路插进来造成跳变。手柄范围收窄到顶栏标题区（不含关闭/队列等按钮，也不含封面/歌词/队列等可滚动区域）——把整屏都做成拖拽区会让内部 `ListView`（歌词/队列）跟外层下滑手势在同一个手势竞技场里打架，全屏级"滚动到顶再接管下滑"的协调是明显更大的功能（`DraggableScrollableSheet` 那种量级），本批不做，只在顶栏这块无滚动内容的安全区开放手柄。
- **B. 打开卡顿——验证结果与用户猜测一致，但多发现一处需要保留不处理的次要贡献者**：读 `CoverFluidBackground` 源码确认第一帧要做四份封面图像合成 + `ColorFiltered` 矩阵 + 64-sigma `BackdropFilter` 全屏模糊，落在转场最需要流畅的那几帧上。修法：`LyricsBackground` 新增 `useFluidBackground` 开关（默认 `true`，`KaraokeScreen` 调用点不受影响），关闭时走新的 `_CheapBackdrop`（用已缓存的封面强调色画一个双色渐变，零图像合成/零滤镜/零 Ticker），跟真实版本共享同一段 `artworkOverlaySkin` 计算逻辑，切换时前景色不跳变；`FullPlayerScreen` 新增 `transition`（`PlayerTransition?`）参数，携带路由自身的 `animation`，单向锁存 `_backdropSettled`（进场动画一旦报告完成就永久为真，不因后续拖拽关闭再翻回假——已经付过一次的建立成本没必要在关闭路径上重付一次）。**取色提前**：`coverPaletteProvider` 是 `autoDispose`，此前只有桌面 shell 的 `_LayoutAccentGradient`（≥1200dp 才存在）持续订阅，窄于 1200dp 的布局下没有任何人预热，每次打开播放页都要现跑一次 `PaletteGenerator`；`MiniPlayerBar`（所有平台常驻）现在也订阅同一个 provider 做纯预热（不使用取值结果），保证不管窗口多宽、播放页一打开就能命中缓存。**验证到的次要贡献者、决定不处理**：`GlassPanel` 自己的 `BackdropFilter`（sigma 18，仅一张图）也会在首帧参与合成，但推迟它意味着转场滑入期间控制面板完全没有毛玻璃质感（比稍重一点的定格帧更明显地"看起来坏了"），本批只处理 `CoverFluidBackground`。**验证到不是问题的猜测**：怀疑过窄屏 `PageView` 会预建歌词页触发 `lyricsProvider`——读 Flutter SDK `page_view.dart` 源码确认默认 `allowImplicitScrolling: false` 时 `scrollCacheExtent` 为 0，非当前页不会被构建，这条路径原本就是安全的，不需要额外处理。**新增测试证明修复生效**：`test/full_player_transition_test.dart` 用短时长 `PageRouteBuilder` 直接驱动 `playerPageTransitionsBuilder`，断言转场进行中树里没有 `CoverFluidBackground`、转场结束后有。
- **C. 进度条 hover 收缩真 bug——根因比最初定位的范围更大一点，两处都修了**：`GlowingSliderThumbShape.getPreferredSize` 此前直接返回 `Size.fromRadius(enabledThumbRadius)`，而 `enabledThumbRadius` 在桌面播放条宽形态下 hover 时从 0 跳到 7——Flutter `BaseSliderTrackShape.getPreferredRect` 按 `max(thumbWidth, overlayWidth)` 在轨道两端各留白，留白量跟着 hover 状态跳变就是轨道跟着"收缩"的根因。拆成 `radius`（当帧实际画的半径，可变）与 `maxRadius`（`getPreferredSize` 恒定返回，不随 hover 变）两个字段，两个调用点（`full_player_screen.dart`、`mini_player_bar.dart`）都改用新签名。**核对时另外发现**：同一行代码相邻的 `RoundSliderOverlayShape(overlayRadius: active ? 16 : 0)`——这是 Flutter SDK 自带的形状，它的 `getPreferredSize` 同样无条件返回 `Size.fromRadius(overlayRadius)`，跟着 `active` 切换 0/16 会喂给上面同一个 `max(thumbWidth, overlayWidth)` 项，是同一类 bug 的第二个来源，只是我们不能改它的源码。改法：`overlayRadius` 恒定传 16，画出来的实际大小交给 Flutter 自己的 hover/press 激活动画（`paint()` 内部按 `activationAnimation` 对 `overlayRadius` 做 tween，不依赖这个字段本身的状态）。测试：新文件 `test/seek_bar_effects_test.dart` 直接单测 `getPreferredSize` 与 `radius` 无关；`mini_player_bar_desktop_test.dart` 新增集成测试，读取 hover 前后真实生效的 `SliderThemeData` 调用同一个 `trackShape.getPreferredRect`，断言轨道宽度完全相同。
- **D. 进度条比例重调（仅桌面播放条宽形态）**：此前进度条与传输键组共享同一个 `_transportBlockWidth`（248，按 5 个传输图标的紧凑排布反推），在约 1200px 宽的桌面播放条上显得又短又局促。拆开两者：新增 `miniPlayerMiddleColumnWidth`（中段列宽 = 该行实测可用宽度的 0.46，钳 [290, 640]，`_wideBreakpoint`=640 处已能容下 248px 传输键组还留余量，1400px 桌面窗口下钳到 640 上限避免中段吃光整行）与 `miniPlayerSeekRowWidth`（进度条宽 = 中段列宽的 0.7，钳 [220, 460]，取用户建议 55%–75% 区间偏宽端，因为进度条自身内容——两个定宽时间标签+间距——已经吃掉一截，实际轨道比这个比例数字看起来更窄）；传输键组维持原有紧凑宽度、居中嵌进新的更宽中段列。**时间标签定宽**：`_timeLabelWidth` 用 `TextPainter` 按当前曲目**总时长**的格式化文本量出所需宽度（而非已播时间自己的文本），已播时间从 `9:59` 跳到 `10:00` 时天然落在同一个预留槽位内，不再挤占轨道；`v5.30.6` 的 `tabularFigures()` 只解决同位数抖动，这次解决的是位数变化本身。测试锁定「同一曲目内已播时间跨位数变化时滑块渲染宽度不变」，另加两组比例函数的边界/夹取单测。
- **E. 封面 hover 放大**：仿 EchoMusic `PlayerBar.vue` 的 `group-hover:scale-110`（`transition-transform duration-500`），新增 `_HoverScaleCover`（`AnimatedScale`，1.08 倍，200ms——取用户给的 1.06–1.10 区间偏轻端，因封面在本仓库播放条里更小更贴近文字，同样的绝对放大量在这个尺度下视觉占比更明显；200ms 而非 EchoMusic 原版 500ms，后者是为点按驱动的过渡设计，hover 反馈套用会显得迟钝），承接 v5.30.7 就已确定的"只有封面能展开播放页"手势范围，仅补视觉提示。
- **调研中发现的两个测试基础设施陷阱，记录供以后参考**：（1）Flutter 测试默认视口只有 800×600（逻辑像素），`SizedBox(width: 900)` 之类请求会被静默钳制到 800——本仓库已有测试长期依赖这个未被察觉的钳制值仍能通过（差值不大，断言恰好两边都成立），本批新增/调整的宽形态阈值测试改用显式 `tester.view.physicalSize` 设定视口，不再依赖钳制后的隐含值。（2）从裸测试代码（而非 widget 的 `build()`/事件回调）里调用 `AnimationController.animateTo()` 后，若紧接着就用 `pump(duration)` 推进模拟时钟，第一次推进的时间会部分耗在 Ticker 校准基准时刻上而不是动画本身——过一遍空 `pump()` 校准 Ticker，再推进时长，是这类测试稳定复现所需要的顺序。
- The phase output is version-tracked and covered by `flutter analyze --no-fatal-infos`（0 error/warning，`player_state_reporter.dart:21` 同一条存量 info 未触碰）and `flutter test`（383/383 通过，364 存量 + 19 新增：`seek_bar_effects_test.dart` 3 例、`mini_player_bar_desktop_test.dart` 净增 8 例、`full_player_transition_test.dart` 1 例、`player_transition_test.dart` 6 例、`mini_player_bar_test.dart` 净增 1 例）。详见 `.plan/20260810-068-v5.32.0-player-transition-and-seek-bar.md`。用户实机验收待跟进：320ms 转场时长/曲线手感、30% 位移与 700px/s 速度两个关闭阈值、进度条最终比例（0.46×0.7）观感。

### v5.31.0 - 2026-08-10

- **feat: 统一设计语言批次开篇——侧栏齐平化 + 中缝渐变层 + macOS 拖动区改走 EchoMusic 形态** — 用户战略决策：放弃追单一 OS 原生保真（v5.30.5/.6/.7 三个版本证明了每修一处就长一道新缝），改用 EchoMusic 式自洽跨平台设计语言；唯一例外是 macOS 红绿灯保持系统绘制。拉取 EchoMusic 源码核对后确认：主界面左右栏 + 播放条在右列内的骨架本来就是对的，割裂感的真正根因是侧栏做成了悬浮面板（外边距 + 圆角 + 阴影）而不是 EchoMusic 那种齐平通铺列。
- **A. 侧栏从悬浮面板改成齐平列**：`_DesktopLayout` 去掉侧栏外层 `Padding`，侧栏贴窗口左、上、下三边；`_DesktopSidebar` 不再用 `GlassPanel`，改成 `DecoratedBox`（通铺 `context.skinColors.surface` 背景 + 仅右侧 `BorderSide` 发丝边框）套一层 `Material(type: MaterialType.transparency)`（保证 `ListTile` 选中态与 ink 仍有 Material 祖先可画——这个坑本仓库在 v5.22.0 和 v5.30.7 的 GlassPanel 版本里各自用不同方式绕过一次，这次换成显式补一层空 Material）。v5.30.7 为"让红绿灯落进悬浮面板"引入的特判——macOS 下 `Padding(0,0,8,8)` 贴边 + 左上角单独取窗口圆角——随悬浮面板一起删除；`GlassPanel.borderRadiusOverride`（该特判专属扩展）确认全仓库除本次删除的调用点外再无其他使用者，退回到删除前的单一 `double borderRadius` 字段。
- **B. macOS 顶部拖动区改用 EchoMusic 形态**：删除按红绿灯精确几何反推顶部内边距的 `_macTitleRowTopInset`（三版本内改了两次，长成了一段多段推导的注释——这本身就是"方法不对"的信号），改成侧栏第一个子节点是一段纯空白拖动区（`DragToMoveArea` 包 `SizedBox`），macOS 48px / 其他桌面平台 24px；品牌行恢复固定的 `EdgeInsets.fromLTRB(16,24,16,12)`（v5.30.0 原始值），不再顶着一个跟红绿灯精确对齐的算出来的数字。拖动区仅在 `DesktopIntegration.isDesktop && !systemTitleBarProvider` 时渲染——系统标题栏开启时那条真实原生栏已经横跨整个窗口宽度并自带拖动，叠加一条自绘的空白拖动区只会是死重的额外留白。
- **C. 中缝渐变氛围层**：新增 `_LayoutAccentGradient`，仿 EchoMusic `MainLayout.vue` 的 `layout-accent-gradient`（该文件原注释明写"横跨侧栏与内容，盖住中缝避免出现分隔白线"）——用 `Stack` 铺在 `_DesktopLayout` 的 `Row` 之上，`IgnorePointer` 包裹保证不吃点击。颜色跟随当前播放曲目的封面取色（`coverPaletteProvider` + `CoverPaletteAccent.accentOverArtwork`，取不到时退回 `context.skinColors.sakuraPink`）——shell 层从未套用过 `artworkOverlaySkin` 那层覆盖皮肤（那套只在播放页/歌词页生效），所以要让这层渐变"跟着封面走"必须直接读取调色板，不能指望 `context.skinColors` 自动带上封面色。高度 240px：实测（非估算）macOS 48px 拖动区场景下侧栏头部堆栈（拖动区+品牌行+账号区+分割线+分组标题）让第一个导航项中心落在 y=226，非 macOS 平台落在 y=202，240 在两种情形下都留出一点余量、让渐变确实"轻微探入"第一行导航项而不是刚好在头部边界切断。透明度按环境皮肤的明暗分两档（深色皮肤 0.24 / 浅色皮肤 0.12，`layoutAccentGradientMaxAlpha` 纯函数、可脱离 widget 直接单测）——依据是同样饱和度的强调色叠在樱花薄暮那种浅色皮肤上会显脏，需要更低的顶部不透明度；这里比对的是**环境皮肤**本身的明暗（因为渐变叠在 shell 自身表面上），不是 `artwork_overlay_skin.dart` 里那个针对封面背景亮度的 `isLightBackdrop`（两者概念不同，那个判断的是播放页封面背景，这里没有对应物）。
- **D. 侧栏底部空白**：齐平化之后自动解决——列表 `Expanded` 之后的空白就是 `DecoratedBox` 自身的通铺背景，跟上方连成一片，不再是悬浮面板边界内的一块孤立空白。本批不往里塞下载/装置入口（排到 v5.32.0）。
- **顺带修复 v5.30.7 造成的回退**：v5.30.7 删除设置页游客态 Account 区块时，误判"侧栏账号区已经有登录入口"对所有断点都成立——实际 `_GuestSignInPrompt` 只存在于 `_DesktopSidebar`（≥1200dp），窄于 1200dp 的访客当时起没有任何 UI 路径退出访客模式。设置页补回一个登录入口，判据用 `ShellChrome.of(context) == null`（只由 `_DesktopLayout` 提供，直接读"是否存在桌面侧栏祖先"，不用窗口宽度硬判——`ShellChrome` 本来就是为这类场景设计的信号）。
- The phase output is version-tracked and covered by `flutter analyze --no-fatal-infos`（0 error/warning，同一条存量 info 未触碰）and `flutter test`（364/364 通过，359 存量 + 5 新增：`shell_scaffold_nav_test.dart` 净增 4——齐平定位/发丝边框方向、拖动区高度 24/48 与品牌行固定内边距、ListTile 失去 GlassPanel 内置 Material 后选中态与 ink 的回归守卫、渐变层横跨双栏且不吃点击；`settings_screen_test.dart` 净增 1，拆成 `ShellChrome` 祖先存在/不存在两种场景验证登录入口的显隐与 `exitGuestMode` 触发）。详见 `.plan/20260810-067-v5.31.0-flush-sidebar-and-seam-gradient.md`。

### v5.30.7 - 2026-08-09

- **fix: 全局 `FilledButtonTheme` 无限宽真 bug（含额外发现的 3 处崩溃点）+ macOS 窗口装饰 + 播放条交互精修** — 用户实机验证 v5.30.6 后给出六条反馈，按优先级处理：一个已定位的真 bug 优先，其余五条按 A-E 顺序。
- **F. `FilledButtonTheme.minimumSize: Size(double.infinity, 48)` 是全仓库 32 处 `FilledButton` 共享的一个真 bug 根因**——Flutter 把这个无限 minWidth 钳制到父级实际约束，落点因父级而异：`ListTile.trailing`（设置页游客态登录按钮）被钳成"吃掉 trailing 分区全部宽度"，`title`/`subtitle` 被挤到几像素宽，逐字换行成竖排（用户截图原样）；`AlertDialog.actions`（约 16 处）被钳回自然宽度，从未受影响；`Column` 非 stretch 场景（约 8 处错误重试/空态按钮）被钳成"占满可用宽度"，视觉上误读成"有意整宽"从未被当 bug 报告；`CrossAxisAlignment.stretch` 祖先（登录页提交按钮等 2 处）整宽本来就来自 stretch 本身，跟这个 bug 无关。排查中额外确认了**第五类、真实存在的崩溃点**：纯 `Row` 里没被 `Expanded` 包裹的子节点（`play_actions_row.dart`"Play"、`local_library_screen.dart`"播放全部"、`user_playlist_detail_screen.dart`"Play All"，共 3 处），Flutter 给这类节点的主轴约束本身就是无限，无限 minWidth 在这里不是被钳制而是直接触发"BoxConstraints forces an infinite width"硬断言——release 构建里断言被 strip 不会崩给用户看，且这三处对应的测试文件此前用裸 `MaterialApp`（未套用本 app 真实主题）跑测试，从未真正跑到这条崩溃路径。修复：`minimumSize` 改为 `Size(0, 48)`（只留高度约束），逐个核对全部 32 处调用点（29 个真实按钮实例）后结论是**没有一处需要在调用点补显式整宽包装**——该整宽的已经靠祖先 `stretch` 拿到，其余要么本不该整宽要么钳制后的自然宽度反而更正确。另按用户"设置里面也不需要登录了"的明确要求，删除设置页游客态整个 Account 登录区块（含区块标题，避免游客态下留孤儿标题），改写指向侧栏账号区入口的注释。**发现但未处理的衔接缺口**：侧栏账号区的登录入口（`_GuestSignInPrompt`）只存在于 ≥1200dp 的桌面布局，Mobile/Tablet 完全没有等价入口——删除设置页登录按钮后，窄于 1200dp 的访客将没有任何 UI 路径退出访客模式，需用户复核是否要单独排期补上。
- **A. macOS 红绿灯落在悬浮侧栏面板内**——根因是侧栏 `Padding(8,8,8,8)` 给面板加了统一外边距，而 macOS 原生红绿灯画在固定窗口坐标，不会跟着 Flutter 的 margin 挪动，面板左上角圆角接缝因此正好骑在红绿灯上。修复为 macOS+自绘标题栏时侧栏外边距改成 `(0,0,8,8)`（左上贴边、右下仍浮空），贴边角的圆角改用 macOS 窗口自身圆角（12，其余三角维持 16，`GlassPanel` 新增可选 `borderRadiusOverride` 支持非统一圆角，不破坏现有的单一 `double` 调用点），标题行顶部内边距从 30 重新推导为 38（两条独立算法路径互相印证：钳制后光沿留白公式、以及"保留贴边前的绝对屏幕位置不变"，均得出 38，未采用任务描述里估算的约 44）。抽出共享谓词统一两处判断逻辑，避免重蹈 v5.30.0 那次红绿灯 bug 的成因（两处各自计算同一个门禁条件，谁都没真的对齐）。顺带排查绿灯（zoom）禁用问题：确认代码逻辑本身无误，但发现并修复了一个真实竞态——`DesktopIntegration._initWindow()` 用一次性同步读取判断登录状态，几乎总是早于 `AuthNotifier.build()` 的两次 `SecureStorage` 异步读取完成，导致已持久化为游客/已登录的用户冷启动时窗口先按"未过闸"打开（不可缩放），修复为改用 `authProvider.future` 加 300ms 超时保护；无法确认这就是用户截图的唯一成因，需实机复核。
- **B. 侧栏选中菜单项贴边**——`_SidebarTile` 的 `ListTile` 没有横向外边距，选中态背景铺满自身边界、跟面板发丝边框在同一批坐标上。加 `Padding(horizontal: 8, vertical: 2)` 收进内缩，同时把 `ListTile` 自己的 `contentPadding` 从继承的 16 改成 8（外边距 8 + 内容内边距 8 = 16，跟 `_SectionHeader`/`_AccountBlock` 的 16 左内边距对齐）。
- **C. 播放条只有点封面才展开播放页**——移除包住整个内容行的外层 `InkWell`，改为封面单独持有跳转手势（`MouseRegion` + `GestureDetector`），标题/歌手各自持有指向详情页的手势（见 D），内部传输/音量/定时器/队列按钮不受影响（它们本就是各自独立的手势，在 gesture arena 里已优先于外层响应）。
- **D. 歌名/歌手超链接到专辑/歌手详情页**——新增共享 widget `HoverLinkText`：`onTap` 为 `null` 时渲染纯文本（无任何手势包装），非空时 hover 显示下划线 + 手型光标，默认不改变文字颜色（避免常驻链接色跟封面驱动的视觉方向冲突）。`PlayerNotifier` 的 `MediaItem.extras` 补上 `artistId`（此前只有 `albumId`），本地（游客）曲目对应分支不带这个字段，`HoverLinkText` 对空 id 天然回退纯文本。播放条与播放页两处都接入，复用已有路由 `AppRoutes.albumDetailPath`/`artistDetailPath`。
- **E. 进度条时间标签间距 + 平滑插值 + 渐变/光晕**——时间标签与滑轨间加 10px 间距。新增 `positionTweenDuration`(260ms) + `TweenAnimationBuilder` 包住三处播放位置滑块（播放条窄/宽两种形状、播放页），在两次 `positionStream` tick 之间线性补间，视觉上从"一跳一跳"变连续；用 `TweenAnimationBuilder` 而非手写 `AnimationController` 是因为它的重新定向（新 tick 打断旧动画）由框架处理、生命周期跟 widget 绑定，天然规避 `cover_fluid_background.dart` 那次"`late final` 延迟初始化导致 dispose 时对失活 element 建 Ticker 崩溃"的教训，不是"记得小心"而是这条路径根本不需要手写 controller。新增 `GradientSliderTrackShape`（active 轨道双色渐变）与 `GlowingSliderThumbShape`（播放中滑块光晕，静态绘制、跟随已有重绘节奏、无独立 Ticker），宽播放条的光晕沿用既有"平时隐形、hover/拖拽才现身"的设计只在滑块可见时叠加。缓冲进度次级轨道未实现——`PlaybackEngine` 抽象接口没有暴露缓冲位置能力，按"拿不到就跳过"处理。
- The phase output is version-tracked and covered by `flutter analyze --no-fatal-infos`（0 error/warning，1 条存量 info 未触碰）and `flutter test`（359/359 通过，341 存量 + 18 新增，含 F 节强制要求的 `ListTile.trailing` 宽度回归测试、额外发现的 3 处 Row 崩溃点回归测试、A/B/C/D 节的 macOS 贴边/圆角/内缩/手势分离/超链接断言）。Mobile/Tablet 访客登录入口缺口、macOS 绿灯竞态修复的实机确认、`_macTitleRowTopInset` 数值三项留待用户实机复核，详见 `.plan/20260809-066-v5.30.7-window-chrome-and-button-theme.md`。
