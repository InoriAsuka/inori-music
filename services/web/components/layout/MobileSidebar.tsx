...
export function MobileSidebar({ open, onClose }: MobileSidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 w-72 transform overflow-y-auto border-r border-[var(--color-border)] bg-[var(--color-surface)] transition-transform duration-200 md:hidden",
          open ? "translate-x-0" : "-translate-x-full"
        )}
      >
        <div className="flex items-center justify-between border-b border-[var(--color-border)] px-4 py-3">
          <span className="font-semibold text-[var(--color-text)]">Inori Music</span>
          <button type="button" onClick={onClose} className="rounded-md p-1.5 hover:bg-[var(--color-surface-raised)]">
            <X size={18} />
          </button>
        </div>
