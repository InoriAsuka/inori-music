/**
 * MobileSidebar — slide-in drawer for mobile viewports.
...
import { X, Music2, Users, Disc3, ListMusic, Heart, History, LayoutDashboard, Search, Shield, Database, Upload, HardDrive, Activity } from "lucide-react";
...
export function MobileSidebar({ open, onClose }: MobileSidebarProps) {
  ...
  return (
    <>
      {/* Backdrop */}
      <div
        className={cn("fixed inset-0 z-40 bg-black/40 transition-opacity md:hidden", open ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none")}
        onClick={onClose}
      />
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 w-72 transform overflow-y-auto bg-[var(--color-card)] transition-transform duration-200 md:hidden",
          open ? "translate-x-0" : "-translate-x-full"
        )}
      >
        <div className="flex items-center justify-between border-b border-[var(--color-border)] px-4 py-3">
          <span className="font-semibold">Inori Music</span>
          <button onClick={onClose} className="rounded-md p-1.5 hover:bg-[var(--color-muted)]"><X size={18} /></button>
        </div>
...
function SectionLabel({ children, className }: { children: React.ReactNode; className?: string }) {
  return <span className={cn("px-2 py-1 text-xs font-semibold uppercase tracking-wider text-[var(--color-muted-foreground)]", className)}>{children}</span>;
}

function NavLink({ item, active }: { item: { href: string; label: string; icon: React.ReactNode }; active: boolean }) {
  return (
    <Link href={item.href} className={cn("flex items-center gap-2 rounded-md px-3 py-2.5 text-sm transition-colors", active ? "bg-[var(--color-primary)] text-[var(--color-primary-foreground)]" : "hover:bg-[var(--color-muted)]")}>
