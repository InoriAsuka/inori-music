      ...
      {open && (
        <>
          ...
          <motion.aside
            ...
            initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ duration: 0.18 }}
          >
            <div className="flex items-center justify-between border-b border-[var(--color-border)] px-4 py-3">
              <h2 className="font-display text-sm font-bold tracking-widest text-[var(--color-primary)]">QUEUE</h2>
              <button onClick={onClose} className="rounded p-1.5 text-[var(--color-text-muted)] hover:bg-[var(--color-surface-raised)] hover:text-[var(--color-text)]"><X size={16} /></button>
            </div>
...
function SortableQueueItem({ track, index, active, onPlay, onRemove }: {
  track: QueueTrack; index: number; active: boolean; onPlay: () => void; onRemove: () => void;
}) {
  const { attributes, listeners, setNodeRef, transform, transition } = useSortable({ id: track.id });
  const style = { transform: CSS.Transform.toString(transform), transition };

  return (
    <div ref={setNodeRef} style={style} className={cn("mb-1 flex items-center gap-2 rounded-lg border border-transparent px-2 py-2 transition-colors", active ? "border-[var(--color-primary)] bg-[var(--color-primary-dim)]" : "hover:bg-[var(--color-surface-raised)]")}>
      <button {...attributes} {...listeners} className="cursor-grab text-[var(--color-text-muted)]"><GripVertical size={14} /></button>
      <button onClick={onPlay} className="min-w-0 flex-1 text-left">
        <p className="truncate text-sm font-medium text-[var(--color-text)]">{index + 1}. {track.title}</p>
      ...
      <button onClick={onRemove} className="rounded p-1 text-[var(--color-text-muted)] hover:text-[var(--color-danger)]"><Trash2 size={13} /></button>
    </div>
  );
}
