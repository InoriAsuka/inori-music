 ...
 *
 * Unlike next.config.test.ts (which calls the config function directly and
 * is fast enough for the normal `npm test` loop), this script runs the
 * REAL command Docker's builder stage runs — `npm run build` — twice, and
 * inspects the REAL build artifact on disk. It's slow (two full production
 * builds), so it is not part of `npm test`; run it explicitly or wire it
 * into CI as its own step.
 *
