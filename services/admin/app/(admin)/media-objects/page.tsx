...
export default function MediaObjectsPage() {
  ...
  async function setLifecycle(moId: string, state: string) {
    ...
    if (data) {
      ...
      setDetail((d) =>
        d ? { ...d, lifecycleLoading: false, mo: { ...d.mo, lifecycleState: updated.lifecycleState } } : null
      );
      await load();
    } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] overflow-hidden">
        ...
        {loading ? (
          ...
          objects.map((o) => (
            <div
              key={o.id}
              className="grid grid-cols-[1fr_120px_90px_auto] items-center gap-0 border-b border-[var(--color-border)] px-4 py-3 last:border-0 hover:bg-[var(--color-surface-raised)] transition-colors cursor-pointer"
              onClick={() => openDetail(o)}
            >
              <div>
                <p className="font-mono text-xs text-[var(--color-text)]">{o.objectKey}</p>
                <p className="font-mono text-xs text-[var(--color-text-muted)]">
                  {o.id.slice(0, 12)}… · {o.backendId.slice(0, 8)}…
                </p>
              </div>
              <span className="font-mono text-xs text-[var(--color-text-muted)]">{o.mimeType}</span>
              <span
                className={`rounded border px-1.5 py-0.5 text-[10px] font-semibold w-fit ${STATE_COLOR[o.lifecycleState] ?? "text-[var(--color-text-muted)] border-[var(--color-border)]"}`}
              >
                {o.lifecycleState}
              </span>
      ...
      {detail && (
        ...
        >
          <div
            className="w-full max-w-xl max-h-[80vh] overflow-y-auto rounded-t-2xl sm:rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6 space-y-5"
          ...
          >
            ...
            <div className="flex items-start justify-between gap-3">
              ...
              <button
                type="button"
                onClick={() => setDetail(null)}
                className="shrink-0 rounded p-1.5 text-[var(--color-text-muted)] hover:text-[var(--color-text)]"
              >
                <X size={16} />
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              {[
                ...
                <div
                  key={k}
                  className="rounded-lg border border-[var(--color-border)] bg-[var(--color-void)] px-3 py-2"
                >
              ...
              <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-void)] px-3 py-2">
                ...
                <select
                  value={detail.mo.lifecycleState}
                  disabled={detail.lifecycleLoading}
            ...
            <div className="flex items-center gap-3">
              <button
                type="button"
                onClick={() => verify(detail.mo.id)}
                disabled={detail.verifying}
              ...
              {detail.verifyResult && (
                <span
                  className={`text-xs font-semibold ${detail.verifyResult === "ok" ? "text-[var(--color-success)]" : "text-[var(--color-danger)]"}`}
                >
                  {detail.verifyResult}
                </span>
              )}
              ...
              {detail.timelineLoading ? (
                ...
                <div className="space-y-1.5 max-h-48 overflow-y-auto">
                  {detail.timeline.map((e, i) => (
                    <div
                      key={i}
                      className="flex items-start gap-2 rounded-lg border border-[var(--color-border)] bg-[var(--color-void)] px-3 py-2"
                    >
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium text-[var(--color-text)]">{e.type}</p>
                        {e.lifecycleState && (
                          <p className="text-xs text-[var(--color-text-muted)]">
                            {e.previousLifecycleState && <>{e.previousLifecycleState} → </>}
                            {e.lifecycleState}
                          </p>
                        )}
