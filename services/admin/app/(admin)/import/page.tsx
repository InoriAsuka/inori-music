...
type CatalogImportRequest = components["schemas"]["CatalogImportRequest"];
...
export default function ImportPage() {
  ...
  async function submitSingle(e: FormEvent) {
    ...
    const body = {
      ...
      ...(single.trackNumber && { trackNumber: Number.parseInt(single.trackNumber) }),
    };
  ...
  async function submitBatch(e: FormEvent) {
    ...
    setError(null);
    setResult(null);
    let items: CatalogImportRequest[];
    try {
    ...
    <div className="space-y-6">
      ...
      {tab === "single" && (
        <form
          ...
          className="max-w-lg space-y-4 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6"
        >
          {[
            { key: "title", label: "Title *", req: true },
            { key: "mediaObjectId", label: "Media Object ID *", req: true },
            ...
            <div key={key} className="space-y-1">
              ...
              <input
                ...
                type="text"
                required={req}
                value={single[key as keyof typeof single]}
          ...
          <button
            type="submit"
            disabled={loading}
            className="flex items-center gap-2 rounded-md bg-[var(--color-primary)] px-4 py-2 text-sm font-semibold text-[var(--color-primary-fg)] hover:opacity-90 disabled:opacity-60"
          >
      ...
      {tab === "batch" && (
        <form
          ...
          className="max-w-2xl space-y-4 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6"
        >
