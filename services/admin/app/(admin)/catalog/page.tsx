...
export default function CatalogPage() {
  ...
  async function load() {
    ...
    if (tab === "artists") {
      ...
      if (data) {
        setRows((data.artists ?? []).map((a) => ({ id: a.id, label: a.name })));
      ...
      if (data) {
        setRows(
          (data.albums ?? []).map((a) => ({
            id: a.id,
            label: a.title,
            sub: a.releaseYear ? String(a.releaseYear) : undefined,
          }))
        );
        setTotal(data.pagination?.total ?? 0);
      }
      ...
      if (data) {
        setRows(
          (data.tracks ?? []).map((t) => ({
            id: t.id,
            label: t.title,
            sub: `${Math.round((t.durationMs ?? 0) / 1000)}s`,
          }))
        );
        setTotal(data.pagination?.total ?? 0);
      }
      ...
      if (data) {
        setRows((data.playlists ?? []).map((p) => ({ id: p.id, label: p.name, sub: `${p.trackIds.length} tracks` })));
        setTotal(data.pagination?.total ?? 0);
      }
    }
    setLoading(false);
  ...
  useEffect(() => {
    load();
  }, [client, tab, offset]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  async function expand(id: string) {
    ...
    if (tab === "artists") {
      ...
      setSubRows(
        ((data as { tracks?: { id: string; title: string; durationMs?: number }[] })?.tracks ?? []).map((t) => ({
          id: t.id,
          label: t.title,
          sub: `${Math.round((t.durationMs ?? 0) / 1000)}s`,
        }))
  ...
  async function save() {
    ...
    if (tab === "artists")
      await client.PATCH("/api/v1/admin/catalog/artists/{id}", {
        params: { path: { id: editing.id } },
        body: { name: editing.label },
      });
    else if (tab === "albums")
      await client.PATCH("/api/v1/admin/catalog/albums/{id}", {
        params: { path: { id: editing.id } },
        body: { title: editing.label },
      });
    else if (tab === "tracks")
      await client.PATCH("/api/v1/admin/catalog/tracks/{id}", {
        params: { path: { id: editing.id } },
        body: { title: editing.label },
      });
    else
      await client.PATCH("/api/v1/admin/catalog/playlists/{id}", {
        params: { path: { id: editing.id } },
        body: { name: editing.label },
      });
    setEditing(null);
  ...
  async function uploadLyrics() {
    ...
    if (error) {
      setLyricsError("Upload failed — check file format (LRC/SRT, UTF-8, <=512KB)");
      return;
    }
    setLyricsFile(null);
    setLyricsTranslationFile(null);
  ...
  return (
    <div className="space-y-6">
          ...
          rows.map((r) => (
            <div key={r.id}>
              <div className="flex items-center gap-3 border-b border-[var(--color-border)] px-4 py-3 hover:bg-[var(--color-surface-raised)] transition-colors">
                <div className="flex-1 min-w-0">
                  {editing?.id === r.id ? (
                    ...
                    <button
                      ...
                      className="truncate text-sm font-medium text-[var(--color-text)] hover:text-[var(--color-primary-hover)] text-left w-full"
                    >
                      {r.label}
                    </button>
                  )}
                ...
                <span className="w-24 truncate font-mono text-xs text-[var(--color-text-muted)]">
                  {r.id.slice(0, 8)}…
                </span>
                <div className="flex items-center gap-1">
                  {editing?.id === r.id && (
                    <Btn onClick={save} title="Save">
                      <Save size={13} />
                    </Btn>
                  )}
                  {tab === "tracks" && (
                    <Btn
                      ...
                      title="Relink media object"
                    >
                      <Link2 size={13} />
                    </Btn>
                  )}
                  {tab === "tracks" && (
                    <Btn onClick={() => openLyrics(r.id)} title="Manage lyrics">
                      <FileText size={13} />
                    </Btn>
                  )}
                  {hasExpand && (
                  ...
                  <Btn onClick={() => del(r.id)} title="Delete" danger>
                    <Trash2 size={13} />
                  </Btn>
                </div>
              </div>
                ...
                <div className="border-b border-[var(--color-border)] bg-[var(--color-void)] px-6 py-3 space-y-2">
                  {subLoading ? (
                    ...
                    <div className="divide-y divide-[var(--color-border)] rounded-lg border border-[var(--color-border)] overflow-hidden">
                      {subRows.map((s) => (
                        <div
                          key={s.id}
                          className="flex items-center gap-3 px-3 py-2 hover:bg-[var(--color-surface-raised)]"
                        >
                          ...
                          {tab === "playlists" && (
                            <button
                              type="button"
                              onClick={() => removePlaylistTrack(r.id, s.id)}
                  ...
                  {tab === "playlists" && (
                    <div className="flex items-center gap-2 pt-1">
                      ...
                      <button
                        type="button"
                        onClick={() => addPlaylistTrack(r.id)}
                        disabled={!addTrackId.trim()}
      ...
      {relinkId && (
        ...
        >
          <div
            className="w-full max-w-sm rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6 space-y-4"
          ...
          >
            ...
            <p className="text-xs text-[var(--color-text-muted)]">
              Track ID: <code className="text-[var(--color-text)]">{relinkId}</code>
            </p>
            <input
              value={relinkMediaId}
      ...
      {lyricsTrackId && (
        ...
        >
          <div
            className="w-full max-w-lg rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6 space-y-4 max-h-[85vh] overflow-y-auto"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between">
            ...
            <p className="text-xs text-[var(--color-text-muted)]">
              Track ID: <code className="text-[var(--color-text)]">{lyricsTrackId}</code>
            </p>
            ...
            {lyricsLoading ? (
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  <span className="rounded-md bg-[var(--color-primary)]/10 px-2 py-0.5 text-xs font-semibold uppercase text-[var(--color-primary)]">
                    {lyrics.format}
                  </span>
                  {lyrics.source && (
                    <span className="rounded-md border border-[var(--color-border)] px-2 py-0.5 text-xs text-[var(--color-text-muted)]">
                      {lyrics.source}
                    </span>
                  )}
                </div>
            ...
            <div className="space-y-2 border-t border-[var(--color-border)] pt-4">
              ...
              <button
                type="button"
                onClick={uploadLyrics}
                disabled={!lyricsFile || lyricsUploading}
                className="w-full flex items-center justify-center gap-1.5 rounded-md bg-[var(--color-primary)] px-4 py-2 text-sm font-semibold text-[var(--color-primary-fg)] hover:opacity-90 disabled:opacity-40"
              >
