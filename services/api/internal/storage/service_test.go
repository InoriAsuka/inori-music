...
func TestServiceSetDefaultBackendFailurePreservesPreviousDefault(t *testing.T) {
	ctx := context.Background()
	switchErr := errors.New("persist default switch")
	repo := newFailingDefaultSwitchRepository(switchErr)
	repo.backends["local-main"] = StorageBackend{
		ID:           "local-main",
		Type:         BackendTypeLocal,
		DisplayName:  "Local",
		Enabled:      true,
	...
	repo.backends["s3-prod"] = StorageBackend{
		ID:           "s3-prod",
		Type:         BackendTypeS3,
		DisplayName:  "S3",
		Enabled:      true,
	...
	_, err := service.SetDefaultBackend(ctx, "s3-prod")
	if !errors.Is(err, switchErr) {
		t.Fatalf("SetDefaultBackend() error = %v, want %v", err, switchErr)
	}
	assertOnlyDefault(t, repo, "local-main")
}

func TestServiceRegisterDefaultBackendFailurePreservesPreviousDefault(t *testing.T) {
	ctx := context.Background()
	switchErr := errors.New("persist registered default")
	repo := newFailingDefaultSwitchRepository(switchErr)
	repo.backends["local-main"] = StorageBackend{
		ID:           "local-main",
		Type:         BackendTypeLocal,
	...
	if !errors.Is(err, switchErr) {
		t.Fatalf("RegisterBackend() error = %v, want %v", err, switchErr)
	}
	assertOnlyDefault(t, repo, "local-main")
	if _, err := repo.Get(ctx, "s3-prod"); !errors.Is(err, ErrNotFound) {
		t.Fatalf("Get(s3-prod) error = %v, want ErrNotFound", err)
	}
}

type failingDefaultSwitchRepository struct {
	backends map[string]StorageBackend
	err      error
}

func newFailingDefaultSwitchRepository(err error) *failingDefaultSwitchRepository {
	return &failingDefaultSwitchRepository{backends: make(map[string]StorageBackend), err: err}
}

func (repo *failingDefaultSwitchRepository) Save(_ context.Context, backend StorageBackend) error {
	if backend.ID == "" {
		return fmt.Errorf("%w: id is required", ErrInvalidBackend)
	}
	if backend.ID == "s3-prod" {
		return repo.err
	}
...
func (repo *failingDefaultSwitchRepository) SaveWithExclusiveDefault(_ context.Context, backend StorageBackend) error {
	if backend.ID == "" {
		return fmt.Errorf("%w: id is required", ErrInvalidBackend)
	}
	return repo.err
...
func (repo *failingDefaultSwitchRepository) Get(_ context.Context, id string) (StorageBackend, error) {
...
func (repo *failingDefaultSwitchRepository) List(_ context.Context) ([]StorageBackend, error) {
	return sortedBackends(repo.backends), nil
}

func (repo *failingDefaultSwitchRepository) ClearDefault(_ context.Context) error {
...
func assertOnlyDefault(t *testing.T, repo Repository, wantID string) {
	t.Helper()
	backends, err := repo.List(context.Background())
	if err != nil {
	...
	if len(defaults) != 1 || defaults[0] != wantID {
		t.Fatalf("defaults = %v, want [%s]", defaults, wantID)
	}
}
