	...
	if q.Genre != "" {
		sql := fmt.Sprintf(`
			SELECT id, title, sort_title, artist_id, COALESCE(album_id,''), media_object_id,
			       track_number, disc_number, duration_ms, COALESCE(genre,''), replay_gain_db, created_at, updated_at,
			       COUNT(*) OVER () AS total_count
			FROM tracks
...
func (r *Repository) queryTracksPage(ctx context.Context, sql string, args ...any) (catalog.ListPage[catalog.Track], error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&t.TrackNumber, &t.DiscNumber, &t.DurationMS, &t.Genre, &t.ReplayGainDb, &t.CreatedAt, &t.UpdatedAt, &total,
		); err != nil {
			return catalog.ListPage[catalog.Track]{}, err
		}
