...
func (r *MemoryRepository) DeletePlayEventsByIDs(_ context.Context, ids []string) (int, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	deleted := 0
	for _, id := range ids {
...
func (r *MemoryRepository) DeletePlayEventsByIDsForUser(_ context.Context, userID string, ids []string) (int, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	deleted := 0
	for _, id := range ids {
		if e, ok := r.events[id]; ok && e.UserID == userID {
			delete(r.events, id)
			deleted++
		}
	}
...
func (r *MemoryRepository) TrackHistoryStats(_ context.Context, f TrackStatsFilter) (TrackHistoryStatsResult, error) {
	...
	for _, e := range r.events {
		...
		listeners[e.UserID] = struct{}{}
		total++
	}
	return TrackHistoryStatsResult{
		TotalEvents:     total,
		UniqueListeners: len(listeners),
	}, nil
}

func (r *MemoryRepository) TrackTopListeners(_ context.Context, f TrackStatsFilter, limit int) ([]UserPlayCount, error) {
	r.mu.RLock()
	counts := make(map[string]int)
	for _, e := range r.events {
		if e.TrackID != f.TrackID {
